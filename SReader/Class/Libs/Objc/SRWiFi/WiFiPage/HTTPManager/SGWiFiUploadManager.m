...
@property(strong, nonatomic) SGWiFiViewController *wifiVC;
@end
...
- (void)showWiFiPageFrontViewController:(UIViewController *)viewController dismiss:(void (^)(void))dismiss {
    self.wifiVC = [SGWiFiViewController new];
    self.wifiVC.dismissBlock = dismiss;
    if (viewController.navigationController) {
        [viewController.navigationController pushViewController:self.wifiVC animated:YES];
    }else{
        [viewController presentViewController:[[UINavigationController alloc] initWithRootViewController:self.wifiVC] animated:YES completion:nil];
    }
}
...
- (void)fileUploadStart:(NSNotification *)nof {
    ...
    [self.wifiVC startUpload:fileName];
    if (self.startBlock) {
...
- (void)fileUploadFinish:(NSNotification *)nof {
    [self.wifiVC stopUpload:_tmpFileName];
    if (self.finishBlock) {
