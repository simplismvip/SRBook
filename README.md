...
#### 介绍
使用Swift完成的电子书完整项目。移动端已经完成了功能但是有好多想加的功能为了赶紧上线放弃或屏蔽了，但是App Store上架图书类需要提供出版资质放弃上架。把iOS端代码开源出来（服务端是Fastapi写的代码暂时不放出来），有需要的同学可以参考下。
...
#### 软件架构
因为都是利用业余时间完成的，无论是电子书阅读器组件的代码结构还是主项目的代码结构都有很大的优化空间。本来想利用版本迭代把未添加功能和代码结构优化下，比如全部使用RxSwift替换优化冗余代码，使用Swinject优化组件通信，抽出来显示显示图书Comp和控制器位单独组件等等。现在没办法上架动力也没有了😂😂😂，不过有想发的同学可以自己搞下！
